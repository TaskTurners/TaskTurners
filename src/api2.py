...
def read_email_from_gmail(email_address, password, mail_id):
    """Reads an email from a Gmail account given its ID."""
    try:
        imapObj = imaplib.IMAP4_SSL('imap.gmail.com')
        imapObj.login(email_address, password)
        imapObj.select('inbox')  # You can choose other mailboxes like 'sent'

        status, data = imapObj.fetch(mail_id, '(RFC822)')  # Fetch the raw email data
        if status != 'OK':
            logging.error(f"Error fetching email with ID {mail_id}: {data}")
            return None

        raw_email = data[0][1]  # The raw email data is at this index
        ...
        logging.error(f"Error reading email from Gmail: {e}")
        return None
...
def extract_text_from_pdf(pdf_path):
    ...
    try:
        ...
        logging.error(f"Error reading PDF: {pdf_path}")
        return None
    except Exception as e:
...
def process_email_route():
    ...
    """
    # Check if the request is for file or Gmail processing
    if 'email_file' in request.files:
    ...
    elif 'email_id' in request.form:  # Assuming you'll send email_id, email_address, and password
        ...
        request_types = ["Adjustment", "Fee Payment", "AU Transfer",
                         "Money Movement-Outbound", "Commitment Change",
                         "Money Movement-Inbound", "Closing Notice"]

        try:
            result = process_gmail_email(email_address, password, email_id, request_types)
            if result:
                return jsonify(result), 200
            else:
                return jsonify({'error': 'Gmail processing failed'}), 500
        except Exception as e:
            logging.error(f"Error processing Gmail email: {e}")
            return jsonify({'error': f'Gmail processing failed: {str(e)}'}), 500
    else:
        return jsonify({'error': 'No email file or Gmail credentials provided'}), 400
...
# Gmail processing function
def process_gmail_email(email_address, password, email_id, request_types):
    """Processes a single email from Gmail and its attachments."""
    try:
        msg = read_email_from_gmail(email_address, password, email_id)
        if not msg:
            return None
        ...
        logging.error(f"Error in process_gmail_email: {e}")
        return None
